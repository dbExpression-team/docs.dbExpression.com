...
Comparison operations within a query must follow the same type check constraints the .NET CLR enforces for any other comparison expression.  For example, if a queried field represents an integer column in your database, comparing that field to a string literal would raise a compile time error.
```csharp
dbo.Person.Id == "4000"
```
